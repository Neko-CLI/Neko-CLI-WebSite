  ...
  content: (
    <>
      ...
      <section id="quick-start">
        <h2>Quick Start ⚡</h2>
        <p>Get Neko-CLI running on your Linux machine in no time!</p>

        <DependenciesModal />

        <h3>Prerequisites:</h3>
        <p>
          You'll need <strong>Node.js</strong> (v16+ recommended) and either{" "}
          <strong>npm</strong> or <strong>Yarn</strong> installed.
        </p>
        <p>
          Check if you have them: <code>node -v</code>, <code>npm -v</code>,{" "}
          <code>yarn -v</code>.
        </p>

        <h3>Installation:</h3>
        <p>Open your terminal and choose your preferred package manager:</p>
        <ul>
          <li>
            <strong>npm:</strong>
        ...
        <p>
          The <code>-g</code> flag ensures a global install, letting you use{" "}
          <code>meow</code> commands anywhere! 🚀
        </p>

        <h3>Verify Install:</h3>
        <p>Confirm it's working:</p>
        <Code>meow --version</Code>
        <p>
          A version number means you're all set! 🎉 Try <code>meow help</code>{" "}
          to explore! 😉
        </p>
      </section>
      ...
      <section id="video-tutorial">
        ...
        <VideoEmbed
          videoId="gW3rS8N2DSc"
          title="Neko-CLI Linux Installation Tutorial"
        />
  ...
  useEffect(() => {
    if (isOpen && loadingNodeVersion) {
      fetch("https://nodejs.org/dist/index.json")
        ...
        .then((data) => {
          const latestLTS = data.find(
            (release) => release.lts && release.lts !== false
          );
          if (latestLTS) {
        ...
        .catch((err) => {
          ...
          setNodeVersionError(
            "Failed to fetch version. Please check nodejs.org."
          );
        })
          ...
          {(onClose) => (
            <>
              ...
              <ModalBody className="text-white">
                <p className="mb-6">
                  To run <strong>Neko-CLI</strong>, you need the following
                  dependencies:
                </p>
